"""Bridge worker main loop (claude_p).

bridge-gemini / bridge-codex と同一パターン:
  - `AgentHub.inbox()` で DM を受信
  - `ClaudePCLIEngine.run()` で `claude -p` を起動
  - claude が MCP tool 経由で agent-hub に返信
  - subprocess 完了後に `hub.ack(msg.id)`

差分 (gemini との比較):
  - engine は Claude Agent SDK ではなく `claude -p` subprocess
  - MCP config は `--mcp-config <tmp_json>` (HOME 分離不要)
  - rate-limit fallback DM なし (M1)
  - workdir missing check あり (issue #51 実績パターン)
  - retry なし (M1)

設計: docs/design-bridge-claude-p.md §5, §8
"""
...
from agent_hub_bridges.claude_p.engine import ClaudePCLIEngine
...
async def run_worker(config: Config) -> None:
    ...
    logger.info(
        "Starting agent-hub-bridge-claude-p as @%s (workdir=%s, tenant=%s)",
        config.user,
        config.workdir,
...
async def _handle_one(
    hub: HubSession,
    engine: ClaudePCLIEngine,
    msg: IncomingMessage,
    config: Config,
) -> None:
    """message 1 件を claude -p に渡し、subprocess 完了で 1 ターン終了とする.
    ...
    try:
        ...
        logger.exception("claude -p error for message %s: %s", msg.id, exc)
        with anyio.move_on_after(10):
            try:
                await hub.send(
                    to=msg.sender,
