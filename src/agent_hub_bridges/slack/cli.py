...
旧 repo の CLI と semantic 同等:
  - `--participant` optional (default `slack-bot`、 env `AGENT_HUB_PARTICIPANT` fallback)
...
def _resolve_participant(cli_value: str | None) -> str:
    """`--participant` の優先順位: CLI > env `AGENT_HUB_PARTICIPANT` > `'slack-bot'` default."""
    return cli_value or os.environ.get("AGENT_HUB_PARTICIPANT") or "slack-bot"
...
def main(argv: list[str] | None = None) -> int:
    ...
    # slack では `--participant` を optional に (default は env or 'slack-bot')。
    ...
    parser.add_argument(
        ...
        help=(
            ...
            " 未指定なら env AGENT_HUB_PARTICIPANT、 それも無ければ 'slack-bot' を使う。"
        ),
    )
