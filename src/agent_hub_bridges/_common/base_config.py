...
# agent-hub auth 用 GitHub PAT の env 名。エコシステム全体 (installer / sdk /
# 各 bridge) で `AGENT_HUB_GITHUB_PAT` に統一済み。`GITHUB_PAT` は旧名で、
# 段階的 deprecation のため当面 alias として受理する (ハード破壊しない)。
_PAT_ENV = "AGENT_HUB_GITHUB_PAT"
_PAT_ENV_LEGACY = "GITHUB_PAT"


def load_github_pat() -> str:
    """agent-hub auth 用 GitHub PAT を読む (必須).

    優先順位:
      1. `AGENT_HUB_GITHUB_PAT` (エコシステム統一名)
      2. `GITHUB_PAT` (deprecated alias — 当面受理、 WARN を出す)

    どちらも無ければ `ValueError` (fail-fast)。
    """
    value = os.environ.get(_PAT_ENV)
    if value:
        return value

    legacy = os.environ.get(_PAT_ENV_LEGACY)
    if legacy:
        logger.warning(
            "env %s is deprecated; rename to %s. The legacy alias is still "
            "accepted for now but will be removed in a future release.",
            _PAT_ENV_LEGACY,
            _PAT_ENV,
        )
        return legacy
    ...
    raise ValueError(
        f"required env var is missing: {_PAT_ENV} "
        f"(deprecated alias {_PAT_ENV_LEGACY} also accepted)"
    )
...
def load_base_config(
    *,
    ...
    Raises:
        ValueError: 必須 env (`AGENT_HUB_URL` / `AGENT_HUB_GITHUB_PAT`) が
            無いか、 workdir が 存在しないディレクトリの場合。
    """
