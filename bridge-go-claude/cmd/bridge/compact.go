...
// getRunner() が nil の間はスキップ (runner lazy init 構造に対応 — issue #91)。
// processing フラグが立っている間はスキップ (stream 競合防止 — issue #102)。
func (w *idleCompactWatchdog) watchAndCompactLazy(ctx context.Context, getRunner func() *claudeRunner) {
	...
	for {
		...
		runner := getRunner()
		if runner == nil {
			// runner 未初期化 — タイマーリセットして次のチェックまで待つ
			w.reset()
			continue
		}
		if w.isProcessing() {
